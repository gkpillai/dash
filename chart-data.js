...
$(document).ready(function () {
  ...
  google.script.run.withSuccessHandler(function (data) {
    ...
    if (months.length > 0) {
      ...
      // Initialize the chart with the last month's data
      var revenueLast = revenues[revenues.length - 1];
      var payrollLast = totalPayrollExpensesIncludingB56[totalPayrollExpensesIncludingB56.length - 1];
      renderPieChart(payrollLast, revenueLast);
    }
...
function renderPieChart(payrollExpensesIncludingB56, revenueData) {
  ...
  chart.render();
    ...
    // Optional: Update the chart when the month is changed from the dropdown
    $('#monthSelector3').change(function () {
      ...
      if (index > -1) {
        ...
        renderPieChart(payrollData, revenueData);
      }
    });
